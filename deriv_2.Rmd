...
\boldsymbol{\beta}^\prime \mathbf{X}^\prime \mathbf{y} &= (\mathbf{y}^\prime \mathbf{X} \boldsymbol{\beta})^\prime \\\\
...
$$
\begin{align}
\text{VAR}\left[\mathbf{e}\right] &= \text{VAR}\left[(\mathbf{y} - \mathbf{H}) (\mathbf{x}\boldsymbol{\beta} + \boldsymbol{\epsilon})\right] \\\\
&= \text{VAR}\left[(\mathbf{y} - \mathbf{H}) \mathbf{\epsilon}\right] \\\\
...
Si $\mathbf{a}$ y $\mathbf{x}$ son vectores de dimensión $p$ ($p \times 1$), entonces

$$
\begin{align}
\nabla (\mathbf{x}^\prime \mathbf{a}) &= a \\\\
