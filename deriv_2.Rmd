...
$$
\begin{align}
t_i &= \frac{e^{(-i)}_i}{\widehat\sigma^2_{(-i)} \sqrt{1 - \mathbf{H}_{ii}}} \\\\
...
&= \frac{e_i^2 \ \mathbf{H}_{ii}}{(p+1) \ (1 - \mathbf{H}_{ii})^2} \\\\
&\sim F_{p+1, \ n-p-1}
\end{align}
$$
...
Lo que nos permite expresar $D_i$ como una distancia de mahalanobis

<br>

$$
D_i = \frac{\left(\widehat{\beta}^{(-i)} - \widehat{\beta}\right)^\prime \ \mathbf{X}^\prime \mathbf{X} \ \left(\widehat{\beta}^{(-i)} - \widehat{\beta}\right)}{(p + 1) \widehat{\sigma}^2}
$$
...
$\frac{2}{n} \sigma^2 (p+1)$ se conoce como el **optimismo** del modelo porque es la cantidad en la que el error cuadrático promedio de la muestra infraestima el error cuadrático promedio de futuras realizaciones de $Y^\prime$.

Si tuviéramos una expresión adecuada del optimismo, tendríamos un estimador insesgado del error cuadrático promedio de futuras realizaciones.
...
$\widehat\sigma^2$ se corresponde con la estimación del error cuadrático promedio del modelo más general entre los que se comparan. $\widehat\sigma^2$ es un estimador insesgado de $\sigma^2$ si dicho modelo subsume el verdadero. La idea es elegir el modelo con menor $C_p$.
...
Maximizar $R^2$ es equivalente a minimizar el error cuadrático promedio ya que 
...
$\nabla \nabla L(\theta^*)$ es la matriz hessiana de nuestras estimaciones, así que, si la definimos con $H$,
...
Si reemplazamos esta última expresión de $\widehat\theta$ por $\widehat\theta$ en
...
Volvamos a hacer una expansión de Taylor, esta vez del logartimo de la verosimilitud del verdadero vector de parámetros.
...
&\approx L(\widehat\theta) + \frac{1}{2} (\theta^* - \widehat\theta)^\prime \ \mathbf{H} \ (\theta^* - \widehat\theta) \\\\
...
Sabiendo que $\text{VAR} \left[ \nabla L(\theta^*) \right] = - \mathbf{h}$ según la identidad de Fisher y volviendo a tomar esperanzas,
...
$$
AIC = L(\widehat\theta) - (p+1)
$$

<br>

$AIC/n$ se corresponde con la esperanza del logaritmo de la verosimilitud de una nueva realización bajo el modelo estimado si dicho modelo es correcto.
