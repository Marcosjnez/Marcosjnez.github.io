...
$$
\text{VIF}_i = \frac{\frac{\sigma^2}{n s_{x_{i}}^2}}{\sigma^2 (\mathbf{X}^\prime \mathbf{X})^{-1}_{i+1, i+1}}
$$
...
\text{VAR}\left[\mathbf{X}\right] &= \mathbf{V} \, \text{diag}(\boldsymbol{\lambda}) \, \mathbf{V}^\prime
\end{align}
$$

Los autovectores de $\text{VAR}\left[\mathbf{X}\right]$ se conocen como los componentes principales de los predictores.

Cualquier vector $a$ puede ser reescrito como una suma de autovectores:
...
Supongamos que estamos interesados en que el error tipo I de $k$ coeficientes sea, conjuntamente, $1 - \alpha$. Una forma común de lograrlo es establecer el error tipo I de cada coeficiente en $\alpha / k$ (o, equivalentemente, multiplicar el valor p por $k$). Sin embargo, esto solo es correcto si los coeficientes son independientes.
...
Por tanto, la probabilidad de que al menos un parámetro no se encuentre en su intervalo de confianza es inferior a $1 - k \alpha$. La correción de Bonferroni es conservadora.
...
Sin embargo, si los coeficientes covarían y la matriz de varianzas-covarianzas es $\boldsymbol{\Sigma}$, entonces
