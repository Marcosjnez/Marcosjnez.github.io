...
$$
\begin{align}
\widehat{\boldsymbol{\beta}} &\sim MVN(\boldsymbol{\beta}, \sigma^2 (\mathbf{X}^T \mathbf{X})^{-1}) \\\\
\widehat{\beta}_i &\sim N(\beta_i, \sigma^2 (\mathbf{X}^T \mathbf{X})^{-1}_{ii}) \\\\
\frac{\widehat{\beta}_i - \beta_i}{\sigma \sqrt{(\mathbf{X}^T \mathbf{X})^{-1}_{ii}}} &\sim N(0, 1) \\\\
...
$$
\begin{align}
\frac{1}{n} \mathbb{E}\left[\mathbf{e}'\mathbf{e}\right] &= \frac{1}{n} \mathbb{E}\left[((\mathbf{I} - \mathbf{H})\mathbf{e})^T (\mathbf{I} - \mathbf{H})\mathbf{e} \right] \\\\
...
$$
\begin{align}
\text{VIF}_i &= \frac{\widehat\sigma^2 / n s_{x_{i}}^2}{\widehat\sigma^2 (\mathbf{X}^T \mathbf{X})^{-1}_{i+1, i+1}} \\\\
...
$$
a = \sum_{i=1}^p (\mathbf{a}^T \mathbf{v}_i) \mathbf{v}_i
$$
...
$$
\widehat{m}(\mathbf{x_i}) = \bar{y} + \frac{1}{n} (\mathbf{x_i} - \mathbf{\bar{x}}) \text{Var}\left[\mathbf{X_c}\right]^{-1} \mathbf{X_c}^T \mathbf{y_c}
$$
...
**Método del cuadrado de los residuos**: Mediante OLS, se computa la media del cuadrado de los residuos ($\mathbf{ee}^T = (\mathbf{y} - \widehat{\mathbf{m}}) (\mathbf{y} - \widehat{\mathbf{m}})^T$).

**Método del logaritmo del cuadrado de los residuos**: Mediante OLS, se computa la media del logaritmo del cuadrado de los residuos ($\hat{s}^2 = \log \left( \mathbf{ee}^T \right)$) y se estima la varianza como $\exp(\widehat{s}^2)$.
