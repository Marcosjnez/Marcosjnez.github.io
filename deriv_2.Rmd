...
&= \mathbb{E}[\bar{x}^2] + \mathbb{E}[\mu^2] - 2\mu \ \mathbb{E}[\bar{x}] \\\\
&= \text{Var}[\bar{x}] + \mathbb{E}[\bar{x}]^2 + \text{Var}[\mu] + \mathbb{E}[\mu]^2 - 2\mu \ \mathbb{E}[\bar{x}] \\\\
&= \text{Var}[\bar{x}] \\\\
...
$$
\begin{align}
\mathbb{E}\left[\sum_{i=1}^n (x_i - \mu)^2\right] &= n \ \mathbb{E} \left[ \bar{x^2} + \bar{\mu^2} - 2\mu \bar{x} \right] \\\\
&= n \left( \text{Var}[X] + \mathbb{E} [\bar{x}]^2 + \text{Var}[\mu] + \mathbb{E} [\bar{\mu}]^2 - 2\mu \mathbb{E} [\bar{x}] \right) \\\\
&= n\sigma^2 \\\\
\mathbb{E}\left[\sum_{i=1}^n (\widehat\mu - \mu)^2\right] &= \sigma^2 \\\\
\mathbb{E}\left[\sum_{i=1}^n (x_i - \mu)(\widehat\mu - \mu) \right] &= n \ \mathbb{E}\left[ \mu^2 + \bar{x}\widehat\mu - \bar{x}\mu - \widehat\mu\mu \right] \\\\
&= n \  \mathbb{E}\left[ \mu^2 + \widehat\mu^2 - 2\widehat\mu\mu \right] \\\\
...
$$
\begin{align}
\sigma^2 &= \frac{1}{n - 1} \ \mathbb{E}\left[\sum_{i=1}^n (x_i - \widehat\mu)^2\right]
\end{align}
$$

<br>

Por tanto, una estimación insesgada de $\sigma^2$ es
...
## Transformación de la realización

<br>

$$
\begin{align}
g(Y) &= \beta_0 + \beta_1X + \epsilon \qquad \epsilon \sim \mathcal{N}(0, \sigma^2) \\\\
Y &= g^{-1}(\beta_0 + \beta_1X + \epsilon)
\end{align}
$$

<br>

Esta transformación puede impedir que $Y$ no siga una distribución gaussiana, que la relación entre $Y$ y $X$ no sea lineal, que su varianza no sea constante para todo $x$ o que los errores no sean aditivos.

Si $g$ se corresponde con la función logarítmica,

<br>

$$
\begin{align}
\log Y &= \beta_0 + \beta_1X + \epsilon \qquad \epsilon \sim \mathcal{N}(0, \sigma^2) \\\\
Y &= e^{\ \beta_0 + \beta_1X} e^{\epsilon} \\\\
&= e^{\ \beta_0 + \beta_1X}  \xi \qquad \xi \sim \log\mathcal{N} \left( e^{\sigma^2/2}, (e^{\sigma^2} - 1) e^{\sigma^2} \right)
\end{align}
$$
...
&= \beta_0 + \beta_1X + \epsilon \qquad \epsilon \sim \mathcal{N}(0, \sigma^2)
\end{align}
$$
...
$$
\begin{align}
\lim_{\lambda \rightarrow 0} \frac{Y^\lambda - 1}{\lambda} &= \lim_{\lambda \rightarrow 0} \frac{\frac{d}{d\lambda} \ Y^\lambda - 1}{\frac{d}{d\lambda} \quad \ \lambda \quad \; \;} \\\\
&= \lim_{\lambda \rightarrow 0} \frac{Y^\lambda \log Y}{1} \\\\
&= \log Y
\end{align}
$$
...
La función ```boxcox``` del paquete ```MASS``` puede utilizarse para estimar el parámetro $\lambda$.

<br>

## Sesgo de estimación por omisión de variables correlacionadas
...
Existen constantes $a_j, \ldots, a_p$ de manera que

$$
\sum_{j=1}^p a_j x_{ij} = 0
$$
para dos filas $i$.
...
$$
\begin{align}
\sum_{i=1}^p a_i \mathbf{x}_i &= 0 \\\\
\mathbf{Xa} &= \mathbf{0} \\\\
\text{Var}\left[\mathbf{Xa} \right] &= \mathbf{a}^T \, \text{Var}\left[\mathbf{X}\right] \mathbf{a} \\\\
&= 0
\end{align}
$$
