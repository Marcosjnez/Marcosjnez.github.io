...
[^1]: En el @OSC2015 se volvieron a contrastar experimentalmente 97 hipótesis reportadas con resultados estadísticamente signifcativos, considerando como replicación la repetición de un p-valor (*p-value*) menor a 0.05. Sin embargo, 
...
```{r loading_stuff, eval = T, echo = T, results = 'hide'}
source('https://raw.githubusercontent.com/betanalpha/knitr_case_studies/master/qr_regression/stan_utility.R')
rstan_options(auto_write = TRUE) # Para guardar los modelos que compilaremos de manera que no tenemos que volver a compilarlos.
options(mc.cores = parallel::detectCores()) # Usamos todos los núcleos del ordenador para computar.
```
...
mydata <- data.frame(id = 1:106) # Identificador del sujeto
...
str(mydata) # Información del dataset
table(mydata$Indicador) # Cantidad de participantes que selecciona cada categoría
```
...
# Contrastamos la asunción de que el coeficiente grupal es cumulativamente constante a lo largo de las categorías (cummulative odds assumption):
odds <- NA; upper.limit <- NA; lower.limit <- NA
for(i in 1:4) {
  binomial_model <- summary(vglm(I(as.numeric(Indicador) <= i) ~ Grupo + Si, family = binomialff(dispersion = 1), data = mydata))@coef3
  odds[i] <- exp(binomial_model["Grupo", "Estimate"])
  upper.limit[i] <- exp(binomial_model["Grupo", "Estimate"] +  binomial_model["Grupo", "Std. Error"] * qnorm(0.975))
  lower.limit[i] <- exp(binomial_model["Grupo", "Estimate"] +  binomial_model["Grupo", "Std. Error"] * qnorm(1-0.975))
}
binomial_coefs <- t(rbind(upper.limit, odds, lower.limit))
colnames(binomial_coefs) <- c("2.5%", "Estimate", "97.5%")
rownames(binomial_coefs) <- paste("Pr(Y) <=", 1:4, sep = " ")
```

```{r Cumulative_odds_assumption2, eval = T, echo = T}
kable(binomial_coefs, digits = 3, caption = "Coeficientes", align = "c") %>%
  kable_styling(bootstrap_options = "striped", full_width = F)
# Las estimaciones de cada parámetro grupal son muy diferentes y presentan una varianza demasiado grande como para asumir su igualdad.
# Por tanto, un modelo cumulativo parcial es más apropiado.
```
...
```{r DAG_2, eval = T, echo = T, message = F, warning = FALSE, fig.align = "center", fig.width = 5, fig.height = 3}
dag2
```
...
$\indent$ Ya que el presente manuscrito no pretende únicamente exponer las distintas filosofías subyacentes a cada método sino proporcionar los medios para que otros investigadores puedan implementarlos, el código de los análisis estadísticos en lenguaje R [@R-base] también es adjuntado.

$\indent$ A continuación se exponen brevemente las principales ideas subyacentes a cada método y se discuten los resultados obtenidos en acordancia.
...
$\indent$ En primer lugar, debemos estimar el efecto de la intervención educativa en el número de administraciones del fármaco. Utilizaremos un modelo betabinomial:
...
# Para ilustrar la sobredispersión primero vamos a ajustar un modelo binomial:
glm.binom <- glm(cbind(Si, No) ~ Grupo, family = binomial, data = mydata)
...
res <- resid(glm.binom, type = "pearson")
```

```{r Overdispersion_2, eval = T, echo = T}
sum(res^2) / glm.binom$df.residual
# Esta gran diferencia indica que la varianza observada es hasta 8.35 veces superior a la esperada por el modelo binomial.
```

```{r Overdispersion_3, eval = T, echo = T}
# Una opción es reajustar el modelo con un parámetro añadido que estime esta sobredispersión (modelo quasibinomial):
summary(glm(cbind(Si, No) ~ Grupo, family = quasibinomial, data = mydata))$dispersion
# Este modelo daría lugar a las mismas estimaciones pero con intervalos de confianza más amplios.
```

```{r Overdispersion_4, eval = F, echo = T}
# Sin embargo, el modelo betabinomial acomoda mucho mejor esta sobredispersión:
...
summary_modelo_XM <- summary(modelo_XM)@coef3 # Tabla de coeficientes en logaritmos de razón de momios.
...
```{r Modelos_2, eval = F, echo = T}
# Los grados de libertad son aproximadamente 1, por lo que utilizaremos un modelo lineal:
...
odds.ratios_XY <- exp(cbind(coef(modelo_XY), confint(modelo_XY))) # Efectos en razón de momios.
summary_modelo_XY <- summary(modelo_XY)@coef3 # Tabla de coeficientes y errores típicos.
```
...
```{r SESOI_2, eval = T, echo = T, warning = FALSE, fig.width = 10, fig.height = 5, fig.align = "center"}
...
```{r Effects_2, eval = T, echo = T, warning = FALSE}
...
```{r Severity_2, eval = T, echo = T, warning = FALSE, fig.width = 10, fig.height = 8, fig.align = "center"}
...
$\indent$ Igualmente, tenemos que computar la evidencia del parámetro $\beta^\prime_{MY}$ comparando las predicciones del modelo logístico cumulativo anterior ($M_2$) con las del mismo modelo sin dicho parámetro ($M_3$). De esta manera, si $\text{BF}_{10} > 1$ y $\text{BF}_{23} > 1$ entonces existe evidencia a favor del modelo $M_1$ y $M_2$, respectivamente, indicando, a su vez, que existe evidencia a favor de la mediación. Sin embargo, la evidencia o factor de Bayes de este efecto mediacional es obtenido a través de la multiplicación de las probabilidades posteriores, para las cuales debemos especificar la credibilidad que concedemos a cada hipótesis ($M_0$, $M_1$, $M_2$, $M_3$$):
...
$\indent$ Siguiendo estos pasos, se estima que los datos obtenidos en el experimento son 518 veces más probables de observar en el modelo betabinomial con el parámetro $\beta_{XM}$ ($M_1$) que en el mismo modelo sin dicho parámetro ($M_0$), es decir, $\text{BF}_{10} \simeq 518$. Por contraparte, $\text{BF}_{23}$ arroja una cifra de 12 dígitos ($5.4 \cdot 10^{12}$), es decir, una evidencia tan grande a favor del modelo ordinal con el parámetro $\beta^\prime_{MY}$ ($M_2$) que podría modificar cualquier creencia a favor del mismo modelo sin dicho parámetro ($M_3$). Una vez obtenidos estos valores, procedemos a computar la probabilidad posterior de $M_0$, $M_1$, $M_2$ y $M_3$ asignando nuestras creencias previas, en forma de probabilidad, en dichas hipótesis. Por ejemplo, podemos mostrarnos escépticos y pensar que $p(M_1) = p(M_2) = 0.25$ y $p(M_0) = p(M_3) = 0.75$, en cuyo caso $$\frac{p(Modelo_{\,1})}{p(Modelo_{\,0})} = \frac{p(Modelo_{\,3})}{p(Modelo_{\,2})} = \frac{1}{3}.$$
...
$$ \frac{p(M_1\, | \,y)}{p(M_0\, | \,y)} = \text{BF}_{10} \, \frac{p(M_1)}{p(M_0)} \simeq \frac{518 \cdot 0.25}{0.75} \simeq 172.6 $$
$$ p(M_1\, | \,y) = \frac{172.6}{1 + 172.6} \simeq 0.994 $$
$$ \frac{p(M_2\, | \,y)}{p(M_3\, | \,y)} = \text{BF}_{23} \, \frac{p(M_2)}{p(M_3)} \simeq \frac{5.4 \cdot 10^{12} \cdot 0.25}{0.75} \simeq 1.8 \cdot 10^{12} $$
$$ p(M_2\, | \,y) \simeq 1 $$
$$\text{probabilidad de mediación} = \frac{p(M_1\, | \,y) \cdot p(M_2\, | \,y)}{1 + p(M_1\, | \,y) \cdot p(M_2\, | \,y)} \simeq 0.994 $$

$\indent$ lo que quiere decir que 
$$\text{BF}_{mediación} \simeq \frac{0.994}{1 - 0.994} \simeq 172.6$$

$\indent$ Estos resultados nos indican que los datos observados son 172 veces más probables bajo la hipótesis mediacional compuesta por $M_1$ y $M_2$ que la no mediacional, compuesta por $M_0$ y $M_3$.

```{r BF1, eval = T, echo = T, warning = FALSE, fig.align = "center", fig.width = 10, fig.height = 5}
...
curve(dnorm(x, 0, 1), xlim = c(-3, 3), main = expression(alpha), ylab = "Densidad")
# Al situar el intercepto en torno a 0 estamos concediendo la misma probabilidad a priori de 0.5 de administrar y no administrar el fármaco en el conjunto de todos los particpantes.
curve(dhalfcauchy(x), xlim = c(-1, 5), main = expression(beta[XM]), ylab = "Densidad")
curve(dunif(x, -1, 1), xlim = c(-1, 1), main = "rho")

curve(dnorm(x, 0, 1), xlim = c(-3, 3), main = expression(alpha[j]), ylab = "Densidad")
# Al situar el intercepto en torno a 0 estamos concediendo la misma probabilidad a priori de 0.5 de seleccionar una categoría menor cuando los predictores están promediados.
curve(dhalfcauchy(x), xlim = c(-1, 5), main = expression(beta*minute[MY]), ylab = "Densidad")
curve(dnorm(x, 0, 5), xlim = c(-15, 15), main = expression(beta*minute[XY[j]]), ylab = "Densidad")
```
...
real intercept;
real beta_Grupo;
real<lower=-1, upper=1> rho; // Correlación[-1, 1]
}
model {
// Declaramos los parámetros de la distribución beta:
real alpha;
real beta;
target += normal_lpdf(intercept | 0, 1);
target += cauchy_lpdf(beta_Grupo | 0, 1) - cauchy_lccdf(0 | 0, 1);
target += uniform_lpdf(rho | -1, 1);
// Aplicamos para cada observación el modelo betabinomial:
for (n in 1:N) {
alpha = inv_logit(intercept - Centered_Grupo[n] * beta_Grupo) * (1 - rho) / rho;
beta = (1 - inv_logit(intercept - Centered_Grupo[n] * beta_Grupo)) * (1 - rho) / rho;
target += beta_binomial_lpmf(Y[n] | 40, alpha, beta); // Likelihood.
}
}
generated quantities {
real Intercept;
Intercept = intercept + mean(Grupo) * beta_Grupo; // Reescalamos el intercepto.
}
"
...
target += uniform_lpdf(rho | -1, 1);
for (n in 1:N) {
alpha = inv_logit(intercept) * (1 - rho) / rho;
beta = (1 - inv_logit(intercept)) * (1 - rho) / rho;
target += beta_binomial_lpmf(Y[n] | 40, alpha, beta);
}
}
"

# Compilamos los modelos:
model_M1 <- stan_model(model_code = M1, model_name="stanmodel") # Puede tardar lo suyo...
...
fit_M1 <- sampling(model_M1, data = data.list, iter = 10000, warmup = 2000, chains = 4, cores = 4, control = list(adapt_delta = .99), seed = 1)
# Tras este comando se visualizará la progresión de las cadenas de montecarlo.
fit_M0 <- sampling(model_M0, data = data.list, iter = 10000, warmup = 2000, chains = 4, cores = 4, control = list(adapt_delta = .99), seed = 1)
...
```{r BF3, eval = F, echo = T}
BF10 <- bridgesampling::bf(ML_M1, ML_M0)$bf # Factor de Bayes

# Si asignamos 3 veces más probabilidades al modelo sin el parámetro de interés, entonces la probabilidad posterior del modelo con dicho parámetro es:
post_prob_M1_M0 <- post_prob(ML_M1, ML_M0, prior_prob = c(.25, .75))
```

```{r BF4, eval = T, echo = T, warning = FALSE}
BF10
post_prob_M1_M0

# Errores de medida en la estimación de los marginal likelihoods:
print(error_measures(ML_M1)$percentage)
print(error_measures(ML_M0)$percentage)
```

```{r BF5, eval = F, echo = T, warning = FALSE, results = "hide"}
...
ML_M2 <- bridge_sampler(fit_M2, method = "warp3", repetitions = 5, silent = TRUE)
ML_M3 <- bridge_sampler(fit_M3, method = "warp3", repetitions = 5, silent = TRUE)
# Utilizando 'method = "normal"' no conseguimos un error de medida aceptable por lo que
# utilizamos "warp3" y repetimos la estimación 5 veces para ver si varía demasiado.
```

```{r BF6, eval = F, echo = T}
# Volvemos a asignar 3 veces más probabilidades al modelo sin el parámetro de interés, entonces la probabilidad posterior del modelo con dicho parámetro es:
BF23 <- bridgesampling::bf(ML_M2, ML_M3)$bf # Factor de Bayes.
...
```{r BF7, eval = T, echo = T, warning = FALSE}
BF23 # Factor de Bayes.
post_prob_M2_M3

# Errores de medida al computar los marginal likelihoods:
error_measures(ML_M2)
...
$\indent$ Los autores del paper original @Barberia2018 realizaron un t-test bayesiano ordinario con varianzas equivalentes entre grupos y reportaron un factor de Bayes. Aunque este método es computacionalmente menos eficiente, también podemos aproximarlo especificando el t-test en Stan y usando bridge sampling. La parametrización de este modelo está en especificado en @Wetzels2009 con la diferencia de presentar las siguientes distribuciones a priori:

$$ \mu \sim \text{Cauchy}(0, 1) $$
$$ \delta \sim \text{HalfCauchy}(0, .7071) $$
$$ \sigma \sim \frac{1}{\sigma} $$

$\indent$ La hipótesis nula descarta el parámetro $\delta$.

```{r Original1, eval = T, echo = T, warning = FALSE}
...
# El factor de Bayes es de 47 a favor de la hipótesis alternativa.
# La parametrización de este modelo ya viene por defecto en la función. Consiste en un prior halfcauchy con escala 0.7071.
```

```{r Original2, eval = F, echo = T, warning = FALSE, results = "hide"}
# Vamos a parametrizar esto en Stan:
Y1 <- data$Causal_Judgment[mydata$Grupo == 0]
Y2 <- data$Causal_Judgment[mydata$Grupo == 1]
n1 <- length(data$Group[mydata$Grupo == 0])
n2 <- length(data$Group[mydata$Grupo == 1])
data.list <- list(n1 = n1, n2 = n2, ratio_1 = n2/nrow(data), ratio_2 = n1/nrow(data), Y1 = Y1, Y2 = Y2, rscale = 1/sqrt(2))

# Hipótesis nula:
Null <- "
data {
int<lower=1> n1; // Número de participantes en la condición de no intervención.
int<lower=1> n2; // Número de participantes en la condición de intervención.
vector[n1] Y1; // Observaciones en la condición de no intervención.
vector[n2] Y2; // Observaciones en la condición de intervención.
real<lower=0> rscale; // Escala de la distribución cauchy.
}
parameters {
real mu;
...
model {
target += cauchy_lpdf(mu | 0, rscale); // Cauchy prior en la media.
...
target += normal_lpdf(Y2 | mu, sigma); // likelihood
}
"
...
real<lower=0, upper=1> ratio_2;
real<lower=0> rscale; // Cauchy prior scale
}
parameters {
real mu;
...
model {
target += cauchy_lpdf(delta | 0, rscale) - cauchy_lccdf(0 | 0, rscale); // Constrained Cauchy prior on delta
// target += cauchy_lpdf(delta | 0, rscale); // Cauchy prior on delta
target += cauchy_lpdf(mu | 0, rscale); // Cauchy prior en delta
target += log(1/sigma); // Jeffreys prior en la desviación típica.
target += normal_lpdf(Y1 | mu + delta * sigma * ratio_1, sigma); // likelihood para el grupo de no intervención.
target += normal_lpdf(Y2 | mu - delta * sigma * ratio_2, sigma); // likelihood para el grupo de intervención.
}
"
...
```{r Original3, eval = T, echo = T, warning = FALSE}
print(error_measures(ML_NULL)$percentage)
print(error_measures(ML_Alternative)$percentage)

BF_Alternative_Null <- bridgesampling::bf(ML_Alternative, ML_NULL) # Factor de Bayes.
BF_Alternative_Null
```

$\indent$ En mi opinión, es muy difícil que las hipótesis que un investigador quiere contrastar estén disponibles por defecto en los paquetes estadísticos disponibles. Sin embargo, usando *brigde sampling* podremos especificar nuestra hipótesis como queramos.
...
$\indent$ En este caso simplemente queremos obtener la distribución de probabilidad posterior de los parámetros condicionando en los datos observados $p(\theta \, | \, y)$. Por tanto, tan solo debemos volver a ajustar los anteriores modelos. Sin embargo, esta vez utilizaremos el bloque denominado *generated quantities* para simular los datos necesarios para comprobar la adecuación del modelo a los datos observados.

$\indent$ Utilizando las mismas distribuciones a priori que en la sección anterior, en la siguiente figura se pueden observar las distribuciones posteriores de los efectos. Dado que la mayoría de modelos engloba integrales analíticamente intratables, estas distribuciones suelen aproximarse aplicando técnicas de montecarlo con las que obtenemos muestras. Es así como podemos conocer la probabilidad de que el efecto indirecto de la administración del fármaco sea mayor o menor que 0, pues tan solo debemos calcular la proporción de muestras que es superior o inferior a 0 para cualquier categoría:
...
```{r Bayesian_analysis_2, eval = F, echo = T, warning = T, fig.align = "center"}
betabinomial_chains <- mcmc_trace(as.array(fit_modelo_betabinomial), pars = c("Intercept", "beta_Grupo"))

betabinomial_log_lik <- extract_log_lik(fit_modelo_betabinomial) # Extraemos el logaritmo del likelihood.
# Computamos LOO, que es mejor que WAIC (según los expertos) y es una medida que podemos usar para comparar modelos. Es como AIC, cuanto menor sea, mayor valor predictivo.
...
```{r Bayesian_analysis_3, eval = T, echo = T, warning = T, fig.align = "center"}
...
# Comprobamos si las cadenas de montecarlo se comportan bien:
betabinomial_chains
print(betabinomial_loo, digits = 3)
print(betabinomial_waic, digits = 3)

# Distribución promediada en ambas condiciones:
curve(dbeta(x, shape1 = mean(betabinomial_alpha) , shape2 = mean(betabinomial_beta)), ylim = c(0, 1.6), lwd = 5)
# Distribución de la probabilidad de administrar el fármaco para cada condición:
for(i in 1:2) curve(dbeta(x, shape1 = unique(betabinomial_alpha)[i], shape2 = unique(betabinomial_beta)[i]), add = T)
# Podemos comprobar que uno de los grupos, el que no recibe la intervención, presenta mayor probabilidad de administrar el fármaco.
...
# Nuestro modelo describe bien el proceso que ha generado nuestros datos.
```

```{r Bayesian_analysis_4, eval = F, echo = T, warning = FALSE, results = "hide"}
ordinal <- "
functions { 
real cumulative_logit_lpmf(int Y, real eta1, real eta2, real eta3, real eta4, vector c) {
vector[5] p; 
...
```{r Bayesian_analysis_6, eval = T, echo = T, warning = T, fig.align = "center"}
...
# Puede que no sea deseable que sean estrictamente similares dado que correríamos el riesgo de hacer overfitting.
...
```{r Bayesian_Effects_1, eval = F, echo = T, warning = FALSE}
...
```{r Bayesian_Effects_2, eval = T, echo = T, warning = FALSE, fig.width = 8, fig.height = 12, fig.align = "center"}
...
```{r Bayesian_Effects_4, eval = T, echo = T}
kable(t(prob_NIE_above_0), digits = 5, align = "c") %>%
  kable_styling(bootstrap_options = "striped", full_width = F)
```

$\indent$ Por tanto, parece razonable concluir que esta variable mediacional aumenta la probabilidad de puntuar en las categorías 1 y 2 en detrimento de las categorías 4 y 5, coincidiendo también con los resultados de los análisis anteriores.
