...
curve(dnorm(x, 0, 1), xlim = c(-3, 3), main = expression(alpha), ylab = "Densidad", xlab = NULL)
...
curve(dhalfcauchy(-x), xlim = c(-5, 1), main = expression(beta[XM]), ylab = "Densidad", xlab = NULL)
curve(dunif(x, 0, 1), xlim = c(0, 1), main = "rho", xlab = NULL)

curve(dnorm(x, 0, 1), xlim = c(-3, 3), main = expression(alpha[j]), ylab = "Densidad", xlab = NULL)
...
curve(dhalfcauchy(-x), xlim = c(-5, 1), main = expression(beta*minute[MY]), ylab = "Densidad", xlab = NULL)
curve(dnorm(x, 0, 5), xlim = c(-15, 15), main = expression(beta*minute[XY[j]]), ylab = "Densidad", xlab = NULL)
```
...
$$ \frac{p(M_1\, | \,y)}{p(M_0\, | \,y)} = \text{BF}_{10} \, \frac{p(M_1)}{p(M_0)} \simeq 518\cdot \frac{0.25}{0.75} \simeq 172.6 $$
```{r BF_6, eval = T, echo = T}
BF10 * (1/3)
```

$$ p(M_1\, | \,y) \simeq \frac{172.6}{1 + 172.6} \simeq 0.994 $$
...
$$ \frac{p(M_2\, | \,y)}{p(M_3\, | \,y)} = \text{BF}_{23} \, \frac{p(M_2)}{p(M_3)} \simeq 5.4 \cdot 10^{12} \cdot \frac{0.25}{0.75} \simeq 1.8 \cdot 10^{12} $$
...
$\indent$ El factor de Bayes es de 47 a favor de la hipótesis alternativa.
...
$\indent$ Visualizamos LOO, WAIC, las distribuciones de probabilidad predichas por el modelo y un par de *posterior predictive checks*:
...
curve(dbeta(x, shape1 = unique(betabinomial_alpha)[1], shape2 = unique(betabinomial_beta)[1]), ylim = c(0, 2), col = "black", ylab = "Density", xlab = "Probability", lwd = 0)
...
$\indent$ Comprobamos si las cadenas de Markov:
...
$\indent$ Visualizamos LOO, WAIC, la distribución posterior de los parámetros y *posterior predictive checks*:
